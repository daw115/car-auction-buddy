...
- **AI scoring (`ai/analyzer.py`)** — `analyze_lots(lots, criteria)` is the single entry. Behavior is driven by env vars:
  ...
  - **An absent condition report is not a clean condition report.** `grade_from_items(..., condition_report=False)` starts from 4.6, not 5.0, and returns a `band`. This is measured, not assumed: of 307 real Manheim lots, 280 carry no damage signal at all in the structured fields, yet their true grade averaged 4.58 — 37% were not "Extra Clean" and the worst was 2.2. Starting from 5.0 overstated our grade by 0.42 points system-wide. Copart and IAAI never publish an itemised report, so every one of their lots goes down this path. The numbers, the sample and its limits live in `scoring/calibration.py`; re-measure there when Copart data arrives, because the sample is Manheim-only, 94% BMW+Toyota and skewed high.
  - `valuationsMmr` exposes Manheim's own pricing model and it verifies: `mmrPrice == adjustedValue` in 100% of lots, and `adjustedValue == baseValue + condition + odometer + color` in 98%. So MMR is a post-adjustment valuation and the model is additive — a grade point is worth ~1.7% of base value. Useful for telling a broker what an uncertain grade actually costs in dollars (`calibration.condition_value_usd`).

- **Cross-source analysis (`scoring/analysis.py`)** — grade, data coverage, landed price, market position and blockers for one lot, plus `rank()` and `best_per_source()`. Two things worth knowing before changing it:
  - Every analysis carries a **coverage ratio** alongside the grade. Measured: Manheim supplies 91% of the AutoGrade inputs, Copart and IAAI 36% (4 of 11). Without that second number a ranking always picks Manheim — not because the car is better but because the report is richer.
  - **Announcements must be read even though AutoGrade ignores them.** AutoGrade excludes them correctly (they describe history, not sheet metal), but branded titles live there. A real lot in this dataset — X7 M60i, grade 5.0, 0% duty, priced under MMR — won the ranking until `announcement_flags()` caught "Lemon Law Manuf Buyback". Blockers also cover salvage, flood, TMU and bill-of-sale-only, and `contradictions()` flags a structured flag disagreeing with free text (`hasPriorPaint: false` next to "PPW - PRIOR PAINTWORK").
...
- **Sales agent (`sales/`)** — lead intake, qualification and client conversation. Same split as the offer pipeline: Python computes, the model only writes sentences, a validator rejects anything breaking the rules.
  ...
  - **A lead has two budgets and they are not interchangeable.** `Lead.confirmed_budget_pln` is cash the client has today (trade-in counted only once `trade_in_sold`); `Lead.potential_budget_pln` adds the trade-in regardless. The gate uses **potential** — it answers "is this person worth an hour", and someone with 100k cash plus an 85k Audi is a 185k client whose conversation simply moves slower; counting cash alone parks them. Search and pricing use **confirmed** — they decide what the client actually sees, and quoting cars against money he does not have yet ends in the surcharge we promise never to make. Using one number for both breaks one side or the other, silently. `Lead.waiting_on` derives the same idea for the funnel: an unsold trade-in is a blocker even when nobody typed one into `blocked_by`.
  - `search.py` closes the loop that was missing: the agent scored leads and wrote messages but never **ran a search**. Make, model, year and budget sat in the database while the broker retyped them into the front-page form; `Stage.SZUKANIE` existed in the model and nothing ever set it. `criteria_from_lead()` builds a `ClientCriteria` — budget goes in as `budget_pln_to` (landed PLN), never converted to USD here, because the auction ceiling depends on the US state and only `scoring/budget.py` knows that. `run_search_for_lead()` goes through the same queue as the panel (`api/main._run_job_with_queue`), so results carry the same scoring; a duplicated path would give one car two different scores depending on where you looked. It stops at a candidate list — **picking the cars stays with the broker**, same rule as `sales/offers.py`.
  - Searches live in their own `lead_searches` table, not columns on the lead: one client gets several scrapes (budget revised, auction lost) and overwriting would erase what was already shown. The row is written *before* the scrape starts, so the broker sees "running" instead of an empty list, and failures are recorded rather than raised — a background task has nobody to raise to, and a lead stuck on "running" forever looks like work in progress.
  - Watch the two shapes: `_search_summary` (embedded in inbox items and lead detail) returns `candidate_count` as a number; `GET /api/sales/leads/{id}/candidates` returns `candidates` as the array of lots. Same concept, different field names **on purpose** — one name for both would blow up the frontend only at runtime.
  - `api/sales_routes.py` exposes two routers: `public_router` (the landing-page form and the free VIN checker — open, rate-limited, honeypot) and `router` (broker views), gated at `include_router` time in `api/main.py`. Add new broker endpoints to `router` so they inherit the token check.
  - `sales/gate.py` decides **who reaches a human**, which is a separate question from `qualification.py`'s "how good is this lead". The thresholds come from unit economics, not taste: at $15k basic the commission is 3 615 zł against a 3–6 k zł acquisition cost, so a mass-market lead loses money, while at $80k premium it is 19 373 zł. Hence `MIN_BUDGET_PLN` (default 150 000) and `MIN_SCORE`. Referrals and returning clients bypass the budget floor — their CAC is zero, so the arithmetic behind the gate does not apply, and rejecting a referral damages the source it came from.
