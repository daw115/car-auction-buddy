...
//
// Self-hosting: the target is a plain Node server, not Cloudflare Workers. The wrapper only
// sets Nitro's `defaultPreset`, so the explicit `nitro.preset` below wins. The MCP plugin was
// removed with the /mcp routes — it hard-imports `cloudflare:workers`, which cannot bundle for
// Node, and it exposed an unauthenticated public endpoint we do not want on the open internet.
