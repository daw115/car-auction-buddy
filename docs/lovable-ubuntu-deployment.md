...
| ------------------- | ------ | --------------------------------------------------------------------------------------------- |
| `API_BASE_URL`      | legacy | Obecna integracja FastAPI; sposób ochrony wymaga discovery. Zastąpi ją `UBUNTU_API_BASE_URL`. |
...
> Zmienne legacy pozostają skonfigurowane jako mechanizm pełnego rollbacku podczas migracji. Selektor nie miesza transportów w ramach żądania: komplet czterech envów Ubuntu wybiera Ubuntu, a całkowity brak tych envów wybiera legacy.
