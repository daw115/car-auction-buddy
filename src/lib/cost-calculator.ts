...
export type CostInputs = {
  ...
  /**
   * Stawki podatkowe z backendu (`/api/public/vin-check` → `pricing/tariff.py`).
   *
   * NIE liczymy ich tutaj. Od 1 lipca 2026 cło zależy od kraju montażu — auto
   * złożone w USA wchodzi na 0%, reszta na 10% — a rozstrzyga to pierwszy znak
   * VIN-u. Ten plik tego nie wie i wiedzieć nie powinien: przepis zmieniał się
   * dwa razy w ciągu roku, a druga kopia prawa rozjeżdża się po cichu. Backend
   * ma jeden moduł, który je zna, i to on odpowiada na to pytanie.
   *
   * Brak wartości znaczy „nie znamy VIN-u", więc liczymy DROŻEJ: cło 10%.
   * Zawyżoną wycenę można obniżyć po sprawdzeniu VIN-u, zaniżona wraca do
   * klienta jako dopłata.
   */
...
export type CostBreakdown = {
  ...
  customs_duty_usd: number; // cło: 0% albo 10%, zależnie od kraju montażu
...
// Awaryjne oszacowanie akcyzy, gdy nie znamy VIN-u. Prawdziwe stawki liczy
// backend (`pricing/tariff.py`) i one mają pierwszeństwo — ta tabela nie zna
// ani preferencji dla hybryd do 3500 cm³, ani interpretacji dla miękkich hybryd.
// Akcyza w PL:
...
export function calculateCost(
  ...
  if (input.transport_override_usd != null)
    notes.push(`Transport: override ${input.transport_override_usd} USD`);
  ...
  // Cło od (cena auta + transport). Stawka przychodzi z backendu, bo zależy od
  // kraju montażu; bez VIN-u zostaje 10%, czyli wariant droższy.
  const dutyPct = input.duty_rate_pct ?? 10;
  const dutyBase = carPrice + transport;
  const customs = dutyBase * (dutyPct / 100);
  notes.push(
    input.duty_rate_pct == null
      ? "Cło 10% — kraj montażu nieustalony. Zdekoduj VIN: auto złożone w USA ma 0% i cena spada."
      : `Cło ${dutyPct}% (ze stawek backendu dla tego VIN-u)`,
  );

  // Akcyza od (cena + transport + cło)
  const excisePct = input.excise_rate_pct ?? exciseRatePct(input.engine_cc, input.fuel);
  const exciseBase = carPrice + transport + customs;
  const excise = exciseBase * (excisePct / 100);
  notes.push(
    input.excise_rate_pct == null
      ? `Akcyza ${excisePct}% (silnik: ${input.engine_cc ?? "?"} cm³, paliwo: ${input.fuel ?? "?"}) — stawka szacowana`
      : `Akcyza ${excisePct}% (ze stawek backendu dla tego VIN-u)`,
  );
  ...
  const subtotalBeforeMargin =
    carPrice + repair + transport + PORT_FEES_USD + customs + excise + vat + HOMOLOGATION_USD;
