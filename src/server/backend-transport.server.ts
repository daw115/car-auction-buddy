...
function terr(
  ...
  message: string,
  body?: unknown,
  code?: BackendTransportError["code"],
): BackendTransportError {
  return { status, message, body, code };
}
...
/** Report whether the currently selected transport has a complete configuration. */
export function isBackendTransportConfigured(): boolean {
  try {
    const transport = selectBackendTransport();
    if (transport === "ubuntu") return true;
    return !!process.env.API_BASE_URL && !!process.env.API_BEARER_TOKEN;
  } catch {
    return false;
  }
}
...
/**
 * Non-throwing variant for operational failures. Configuration errors remain
 * fail-closed so a partial Ubuntu rollout cannot silently look healthy.
 */
export async function backendRequestSafe<T>(req: BackendRequest, fallback: T): Promise<T> {
  try {
    return await backendRequest<T>(req);
  } catch (error) {
    if ((error as BackendTransportError | undefined)?.code === "incomplete_configuration") {
      throw error;
    }
    return fallback;
  }
