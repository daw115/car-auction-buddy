...
const styles = StyleSheet.create({
  ...
  // Ponad budżet to nie odrzucenie: auto ma realną ocenę, nie pasuje tylko cena.
  badgePonadBudzet: { backgroundColor: "#fdf3e3", color: "#8a5a13" },
...
function badgeStyle(rec: string) {
  ...
  if (rec === "PONAD BUDŻET") return styles.badgePonadBudzet;
      ...
      ...lots.map((al, idx) => {
        ...
        return React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              mode === "broker" &&
                React.createElement(
                  Text,
                  { style: [styles.badge, badgeStyle(a.recommendation)] },
                  a.recommendation,
                ),
              React.createElement(
                Text,
                { style: [styles.scoreBox, { marginLeft: 8 }] },
          ...
          React.createElement(
            ...
            field(
              "Lokalizacja",
              `${lot.location_city ?? ""} ${lot.location_state ?? ""}`.trim() || "—",
            ),
            field(
              "Uszkodzenie",
              `${lot.damage_primary ?? "—"}${lot.damage_secondary ? ` / ${lot.damage_secondary}` : ""}`,
            ),
            field("Tytuł", lot.title_type ?? "—"),
            field(
              "Aktualna oferta",
              lot.current_bid_usd ? `$${lot.current_bid_usd.toLocaleString()}` : "—",
            ),
          ...
          React.createElement(
            ...
            React.createElement(
              ...
              mode === "broker" ? "Analiza brokera" : "Opis",
            ),
            React.createElement(
              Text,
          ...
          React.createElement(
            ...
            mode === "broker"
              ? costRow("Naprawa (szac.)", `$${cost.repair_usd.toLocaleString()}`)
              : null,
            costRow(
              `Transport (${cost.region})`,
              `$${cost.transport_usa_to_pl_usd.toLocaleString()}`,
            ),
            costRow(
              "Cło + akcyza + VAT",
              `$${(cost.customs_duty_usd + cost.excise_tax_usd + cost.vat_usd).toLocaleString()}`,
            ),
            costRow(
              "Opłaty + homologacja + marża",
              `$${(cost.port_fees_usd + cost.homologation_usd + cost.broker_margin_usd).toLocaleString()}`,
            ),
            React.createElement(
              View,
