...
describe("backendRequestSafe — configuration and text responses", () => {
  it("does not swallow a partial Ubuntu configuration", async () => {
    process.env.UBUNTU_API_BASE_URL = UBU_URL;
    setLegacyEnv();
    const fetchMock = vi.fn();
    globalThis.fetch = fetchMock as unknown as typeof fetch;

    await expect(backendRequestSafe({ path: "/api/jobs" }, { jobs: [] })).rejects.toMatchObject({
      code: "incomplete_configuration",
    });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("supports bounded text responses through the Ubuntu transport", async () => {
    setUbuntuEnv();
    ...
    await expect(
      backendRequest<string>({ path: "/report/client", responseType: "text" }),
