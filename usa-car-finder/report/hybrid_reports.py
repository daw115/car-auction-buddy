...
# Parser mieszka w `ai/json_modelu.py` — używa go też agent ofert. Trzy kopie
# tej samej wiedzy już raz kosztowały 34 niewygenerowane raporty.
from ai.json_modelu import (  # noqa: E402
    OdpowiedzNieDoOdczytania,
    _parse_json_loose,
    _sanitize_llm_json,
    _strip_json,
)
...
def _call_claude_code_json(system: str, user: str, max_tokens: int = 1500) -> dict:
    """Fragmenty raportu przez Claude Code (subskrypcja, bez klucza API).

    Prompt systemowy jest tu stały (CLIENT_SYSTEM / BROKER_SYSTEM / PAIR_SYSTEM), więc
